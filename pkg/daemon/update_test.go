		...
		t.Run(fmt.Sprintf("case#%d", idx), func(t *testing.T) {
			...
			rMock := RpmOstreeClient{
				...
				runRpmOstreeFunc: func(noun string, args ...string) ([]byte, error) {
					// mock the default which is to return the straight kargs.
			...
			// Test logic for determining if the kernel args are in use.
			for _, k := range test.realKargs {
				if inUse, _ := rMock.isKernelArgInUse(k); !inUse {
					t.Errorf("Kernel argument %s should be reported as in use.", k)
				}
			}

			// Ensure that the expected rpm-ostree karg is as expected.
